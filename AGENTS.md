...
just docs                    # after a step declaration changes: regenerates docs/steps.md
```
...
4. Run `just docs` when a declaration changed, then `just check`. When a gate
...
- Every relative link resolves, every backticked test name exists, and every
  input, output and secret has its row. `just docs` writes `docs/steps.md`;
