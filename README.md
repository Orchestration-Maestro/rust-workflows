...
`rust-gate sync` writes `mise.toml`, `mise.lock` and `scripts/bootstrap.sh` into
every other repository of the organization as managed files, so a contributor
anywhere runs these tools at the versions CI runs: `./scripts/bootstrap.sh`
installs them and the commit hooks, with or without a justfile. Pins move here
alone, through the weekly `tool-updates.yml`, and reach the others with the next
release; see [managed files](docs/ci.md#managed-files).
