...
**Opt-in gate**: A gate a caller switches on. All but five default to off or
permissive, so upgrading never fails a consumer who did not ask for it; mutation
testing, the unused-dependency check, the `unsafe` ban, SARIF reports and public
API compatibility default to on because a golden workflow enforces the standard, and each is one input to switch off. The dependency source policy and the scaffolding lints
