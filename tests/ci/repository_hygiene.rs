...
fn report(f: &Fixture) -> String {
    fs::read_to_string(f.root.join("reports/hygiene.txt")).unwrap()
}
...
fn unlinked_markers_in_comments_are_refused_and_linked_ones_pass() {
    ...
    refused(&f.run("ci", "hygiene"), "hygiene: 2 findings");
    assert_eq!(
        report(&f),
        concat!(
...
fn snapshots_large_files_modes_and_links_are_refused() {
    ...
    symlink("/tmp", f.root.join("outside")).unwrap();
    symlink("nowhere", f.root.join("dangling")).unwrap();
    git(&f, &["add", "--", "outside", "dangling"]);
    refused(&f.run("ci", "hygiene"), "hygiene: 6 findings");
    assert_eq!(
        report(&f),
        concat!(
    ...
    .unwrap();
    refused(&f.run("ci", "hygiene"), "hygiene: 5 findings");
    assert!(
        report(&f).contains("EXCUSED HYG-003 big.bin:"),
        "{}",
        report(&f)
    );
}
...
fn a_readme_a_licence_and_a_changelog_are_required() {
    let f = checkout(&[(".github/release-please/config.json", "{}\n")]);
    git(&f, &["rm", "-q", "--cached", "README.md", "LICENSE"]);
    refused(&f.run("ci", "hygiene"), "hygiene: 3 findings");
    assert_eq!(
        report(&f),
        concat!(
...
fn wide_shell_lines_and_justfiles_are_refused() {
    ...
    let f = checkout(&[("justfile", &wide), ("scripts/run.sh*", &script)]);
    refused(&f.run("ci", "hygiene"), "hygiene: 2 findings");
    assert_eq!(
        report(&f),
        concat!(
