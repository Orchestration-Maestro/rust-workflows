...
fn init_writes_every_managed_file_and_the_check_finds_them_equal() {
    ...
    // The toolbelt is the home's: mise.lock byte for byte, since mise reads
    // it back, and the bootstrap under the header, after its shebang, and
    // executable.
    let home = |path: &str| fs::read_to_string(root().join(path)).unwrap();
    assert_eq!(managed(&fixture, "mise.lock"), home("mise.lock"));
    assert!(managed(&fixture, "mise.toml").ends_with(&home("mise.toml")));
    assert_eq!(
        managed(&fixture, "scripts/bootstrap.sh"),
        home("scripts/bootstrap.sh").replacen(
            "bash\n",
            "bash\n# generated by rust-gate sync; do not edit\n",
            1
        )
    );
    let script = fs::metadata(fixture.root.join("project/scripts/bootstrap.sh")).unwrap();
