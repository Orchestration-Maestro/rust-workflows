...
fn a_private_workspace_whose_members_depend_on_each_other_builds_without_packaging() {
    ...
    // package: Cargo looks the dependency up on crates.io. None of them can be
    // published either, so nothing is packaged, not even on an older compiler
    // that would need Cargo 1.90, and the step says so; the release tests,
    // the build and the SBOMs still run.
    let mut fixture = Fixture::new();
...
fn a_mixed_workspace_packages_only_the_members_that_may_be_published() {
    // A member whose `publish` is unset or names a registry may be published
    // and is packaged; one with `publish = false` is left out, even when it
    // depends on a member that is packaged.
    let mut fixture = Fixture::new();
    let metadata = workspace_metadata(
        &fixture,
        &[
    ...
    assert_eq!(
        packaged,
        ["cargo package --locked --package maestro-arithmetic --package maestro-checked"],
        "{trace}"
    );
}
