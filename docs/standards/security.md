...
| SCH-007 | Reviewed dependency updates | **Deterministic.** Dependabot proposes; nothing auto-merges; the full gate must pass. |
...
| SCH-011 | Published artifact verification | **Met, and honest about its limit.** [SECURITY.md](../../SECURITY.md) documents checksums, attestation identity and subject, both SBOM formats and the embedded dependency list. It states that no release has been published yet, so the procedure is unexercised. |
