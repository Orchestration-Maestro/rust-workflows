...
| Merge queue | A repository ruleset `merge-queue` on every repository, its `merge_queue` rule on the default branch with merge method squash, every entry held green and one group built at a time; GitHub accepts that rule in no organization ruleset. Each merge group's run then files the code scanning and Codecov baselines on the default branch; see [uploads](ci.md#uploads-to-code-scanning-and-codecov). The queue waits for every required check to report on the group's commit, so no required check may come from a workflow that does not run on `merge_group`. |
