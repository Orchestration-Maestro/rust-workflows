...
| Merge queue | An organization ruleset with the `merge_queue` rule on each default branch, merge method squash, so each merge group's run files the code scanning and Codecov baselines on the default branch; see [uploads](ci.md#uploads-to-code-scanning-and-codecov). The queue waits for every required check to report on the group's commit, so no required check may come from a workflow that does not run on `merge_group`. |
